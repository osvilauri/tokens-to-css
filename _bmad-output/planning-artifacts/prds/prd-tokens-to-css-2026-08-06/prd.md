...
- **SM-5**: Conversion throughput — a 10k-token accept fixture converts in under **300 ms**, best of three, measured end to end through the Main Entry. **Ratified 2026-08-30 from measurement (OQ-2 closed).** A developer laptop does it in 30 ms; twelve samples on CI across three Node lines land between 46 ms and 83 ms. The bar sits at roughly 3.5× the slowest of those — loose enough that a shared runner never trips it, tight enough that a real regression does. It replaces an inherited guess of 2,000 ms: at sixty-six times the real number that was not a bar, since the emitter could have become sixty times slower and still passed. Enforced in CI as a build failure, judged on the best of three because one CI sample read 301 ms where the same commit read 74 ms twice. Scaling is linear to at least 200k tokens, so the bar does not quietly stop meaning anything at size.
