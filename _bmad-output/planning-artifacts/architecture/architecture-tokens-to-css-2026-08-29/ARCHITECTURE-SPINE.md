...
- **Composite tokens — after the first release.** Decided 2026-08-30. A typography token is five CSS properties, so accepting it means either breaking "one token, one custom property" or dropping a field silently. Neither is a parsing decision. `AD-24` deliberately covers only the object shapes that *are* one value. **Revisit when:** v1 has shipped and there is a real request for it, with the survey and fixtures a PRD revision needs.
- **Everything else in `addendum.md` "Deferred product capabilities"** — CLI, in-memory ingest, multi-file merge, composites, expressions, theme selectors, SCSS/JS emitters. The spine deliberately does not pre-shape for them; AD-1/AD-2 keep the pipeline cheap to extend when a PRD revision arrives.
